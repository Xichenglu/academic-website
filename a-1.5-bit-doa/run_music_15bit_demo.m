...
% This script demonstrates a reproducible simulation pipeline for
% direction-of-arrival (DOA) estimation using low-bit quantization.
% Workflow:
%   1) Generate array snapshots for multiple sources.
%   2) Apply symmetric b-bit quantization (set b=2 for 1.5-bit style test).
%   3) Recover correlation coefficients with a Van Vleck style inversion.
%   4) Reconstruct covariance and estimate DOA via MUSIC.
%
...
b       = 2;             % Quantization bits (b=2 for 1.5-bit experiment)
Norder  = 10;            % Hermite expansion order
maxIter = 300;           % Maximum iterations for inversion
tol     = 1e-6;          % Convergence tolerance
lambda  = 0.5;           % Quantizer threshold factor
...
xlabel('Angle (deg)');
ylabel('|P_{MU}(\theta)|');
title(sprintf('MUSIC Spectrum (b = %d)', b));
